...
from shared.db.model.models import (
    User, Company, JobMaster, JobApplication, 
    ApplicationDocument, FileObject, ApplicationDocumentParsed
)
from pipelines.applicant_evaluation.infrastructure.persistence.doc_repository import SqlAlchemyDocRepository
from pipelines.applicant_evaluation.domain.models.document import ApplicantDocuments, ParsedDoc
...
async def setup_base_data(db_session):
    ...
    # 1. User
    user = User(
        platform_name="KAKAO", email="test@kakao.com", password="pw", 
        nickname="Tester", img_id="img"
    )
    db_session.add(user)
    ...
    job = JobMaster(
        company_id=company.company_id, job_title="Engineer", status="OPEN"
    )
    db_session.add(job)
    await db_session.flush()
...
async def test_get_documents_with_files(db_session, setup_base_data):
    ...
    resume_file = FileObject(
        storage_provider="S3", bucket="bucket", object_key="resume.pdf",
    ...
    assert docs.resume_file.file_path == "resume.pdf" # 저장된 key 확인 (Repo 로직에 따라 다를 수 있음)
...
async def test_get_documents_with_parsed(db_session, setup_base_data):
    ...
    pf_file = FileObject(
        storage_provider="S3", bucket="bucket", object_key="pf.pdf",
        original_name="pf.pdf", size_bytes=200
    )
...
async def test_save_parsed_doc(db_session, setup_base_data):
    ...
    # 1. 파일 정보만 존재 (RESUME)
    resume_file = FileObject(
        storage_provider="S3", bucket="bucket", object_key="r.pdf",
        original_name="r.pdf", size_bytes=100
    )
    db_session.add(resume_file)
    await db_session.flush()
