...
from shared.db.model.models import (
    User, Company, JobMaster, JobApplication, FileObject, ApplicationDocument, AiApplicantEvaluation
)
...
async def pipeline_data(db_session):
    ...
    # 1. User
    user = User(platform_name="TEST", email="pipe@test.com", password="pw", nickname="PipeUser", img_id="img")
    db_session.add(user)
    ...
    job = JobMaster(
        company_id=company.company_id, job_title="Pipeline Engineer", 
    ...
    # 3. Application
    app = JobApplication(user_id=user.user_id, job_master_id=job.job_master_id, status="APPLIED")
    db_session.add(app)
    await db_session.commit() # ID 확정

    # 4. Files (Resume)
    resume = FileObject(
        storage_provider="S3", bucket="bucket", object_key="pipe_resume.pdf", 
        original_name="resume.pdf", size_bytes=100
    )
    db_session.add(resume)
    await db_session.flush()

    app_doc = ApplicationDocument(job_application_id=app.job_application_id, file_id=resume.file_id, doc_type="RESUME")
...
async def test_run_pipeline_end_to_end(db_session, pipeline_data):
    ...
    with patch.object(settings, "USE_MOCK", True):
        ...
        with patch("pipelines.applicant_evaluation.main.S3FileStorage") as MockStorage, \
             patch("pipelines.applicant_evaluation.main.PyPdfExtractor") as MockExtractor:
            ...
            extractor_instance.extract_text.return_value = "Extracted Resume Text Content..."
            ...
            async fn_get_db_override():
                yield db_session
            
            with patch("pipelines.applicant_evaluation.main.get_db", side_effect=fn_get_db_override):
