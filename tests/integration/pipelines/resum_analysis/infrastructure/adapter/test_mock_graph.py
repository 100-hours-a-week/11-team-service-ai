...
from pipelines.resume_analysis.infrastructure.adapters.llm.ai_agent.resercher_graph.graph import (
    TechResearcher,
)
from pipelines.resume_analysis.infrastructure.adapters.llm.ai_agent.configuration import (
    AnalyseContext,
)
...
async def main():
    ...
    job_info = JobInfo(
        ...
        evaluation_criteria=[
            EvaluationCriteria(
                name="Agentic 워크플로우 및 RAG 설계",
                description="LangGraph, LangChain 등을 활용해 복잡한 AI 에이전트 시스템 및 RAG 구조를 설계하고 성능을 개선한 경험",
            ),
            EvaluationCriteria(
                name="인퍼런스/서빙 최적화",
                description="vLLM, TensorRT-LLM 등을 활용하여 모델 추론 속도 향상 및 GPU 메모리 최적화를 이뤄낸 경험",
            ),
            EvaluationCriteria(
                name="AI 퀄리티 문제 해결",
                description="환각(Hallucination) 제어, 프롬프트 엔지니어링, 혹은 파라미터 미세조정(PEFT/QLoRA)을 통한 품질 개선 경험",
            ),
        ],
    )
    ...
    # 에이전트 실행 대기
    result = await researcher.start_researcher(config=config, runtime=context_data)
