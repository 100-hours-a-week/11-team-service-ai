...
from pipelines.resume_analysis.infrastructure.persistence.doc_repository import SqlAlchemyDocRepository

# Import Adapters (Real & Mock)
from pipelines.resume_analysis.infrastructure.adapters.storage.s3_storage import S3FileStorage
from pipelines.resume_analysis.infrastructure.adapters.parser.pdf_extractor import PyPdfExtractor
from pipelines.resume_analysis.infrastructure.adapters.llm.mock_agent import MockAnalyst # Mock Agent
...
async def test_analyze_resume_integration(db_session: AsyncSession):
    ...
    try:
        ...
        if hasattr(response, "ai_analysis_report"): # Pydantic v2
             assert "직무 적합도" in response.ai_analysis_report or "[Mock]" in response.ai_analysis_report or len(response.ai_analysis_report) > 0
...
async def test_analyze_portfolio_integration(db_session: AsyncSession):
    ...
    try:
        ...
        if hasattr(response, "ai_analysis_report"):
            assert "[Mock]" in response.ai_analysis_report or len(response.ai_analysis_report) > 0
