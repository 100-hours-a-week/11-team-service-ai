    ...
    async for session in get_db():
        try:
            ...
            try:
                ...
                if count == 0:
                    pytest.skip(
                        "⚠️ Test data not found. Please run 'tests/test_data_insert.sql'."
                    )
            ...
            if resume:
                print(
                    f"   📄 Resume Found: Path='{resume.file_path}', Type='{resume.file_type}'"
                )
            ...
            if parsed_resume:
                ...
                assert (
                    "Backend Developer" in parsed_resume.text
                    or "Java" in parsed_resume.text
                ), "Resume text mismatch"
            else:
            ...
            if portfolio:
                print(
                    f"   🎨 Portfolio Found: Path='{portfolio.file_path}', Type='{portfolio.file_type}'"
                )
                assert (
                    portfolio.file_type == "PORTFOLIO"
                ), "Portfolio file type mismatch"
            else:
            ...
            if parsed_portfolio:
                ...
                assert (
                    "Project" in parsed_portfolio.text
                    or "Microservices" in parsed_portfolio.text
                ), "Portfolio text mismatch"
            else:
