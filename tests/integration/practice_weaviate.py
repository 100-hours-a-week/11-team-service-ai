...
def test_search():
    ...
    if not response.objects:
        ...
        for obj in response.objects:
            print(f"Found something: {obj.properties['content']} (Dist: {obj.metadata.distance:.4f})")
            print("=> 만약 결과가 나왔다면, '100'이라는 숫자가 텍스트적으로 해석되어 유사도가 계산된 것임.")

    print("\n--- 🔍 Test 3: Filter by ID (메타데이터 역할 확인) ---")
    # some_id가 200인 것만 필터링
    response = collection.query.fetch_objects(
        filters=Filter.by_property("some_id").equal(200),
        limit=2
    )
    for obj in response.objects:
        print(f"Filtered Result: {obj.properties['content']} (ID: {obj.properties['some_id']})")
