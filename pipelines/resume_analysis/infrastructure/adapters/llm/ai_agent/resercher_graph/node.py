...
from .prompt import EXTRACT_UNKNOWN_TECH_PROMPT, EXTRACT_TECH_FACTOR_PROMPT, EVALUATE_CONTEXT_PROMPT
...
async def extract_unknown_tech_node(
    ...
    result = await chain.ainvoke(
        {
            "job_info": rtx.job_info.model_dump_json(include={"company_name", "main_tasks", "tech_stacks", "summary"}, ensure_ascii=False) if hasattr(rtx.job_info, "model_dump_json") else str(rtx.job_info),
...
async def ai_judge_node(
    ...
    class ValidContextEval(BaseModel):
        is_valid: bool = Field(description="제공된 컨텍스트가 검색 대상을 이해하기에 유효한지 여부 (유효하면 True, 쓸모없으면 False)")
    ...
    # 판별 결과(True/False) 추출
    is_valid = eval_result.is_valid if eval_result else False
    ...
    try:
        ...
        # ingest.py의 ingest_docs 함수는 문서 청킹, 임베딩은 cpu작업이기 때문에 비동기 처리를 위해 서브 쓰레드에서 실행
            try:
                ...
                logger.error(f"[{keyword}] 인덱싱 중 에러 발생 (검색 컨텍스트는 계속 유지): {ingest_error}")
