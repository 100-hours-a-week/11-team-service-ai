...
class JobDuplicateChecker:
    ...
    async def check_existing_post_by_url(
        self, url: str
    ...
    async def check_existing_post_by_fingerprint(
    ...
    async def check_semantic_duplicate(
        self, company_id: int, job_text: str
    ) -> Optional[int]:
        """
        ...
        candidates = await self.job_vector_repo.search_similar_jobs(
            company_id, job_text
        )
        ...
        if similarity >= 0.85:
            ...
            if not existing_content:
                ...
                logger.warning(
                    "⚠️ Existing content not found in vector result. Skipping AI check."
                )
                return None
            ...
            is_same = await ai_agent.is_same_job_posting(
                {"content_summary": new_content}, {"content_summary": existing_content}
            )
