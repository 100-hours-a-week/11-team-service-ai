...
from sqlalchemy import String, Date, DateTime, ForeignKey, BigInteger, Text, UniqueConstraint
...
class CompanyAlias(Base):
    ...
    __table_args__ = (
        UniqueConstraint('company_id', 'alias_normalized', name='uk_company_aliases_company_norm'),
    )

    alias_id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    company_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("companies.company_id"), nullable=False, index=True)
...
class SkillAlias(Base):
    ...
    __table_args__ = (
        UniqueConstraint('skill_id', 'alias_normalized', name='uk_skill_aliases_skill_norm'),
    )

    alias_id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    skill_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("skills.skill_id"), nullable=False, index=True)
...
class JobMasterSkill(Base):
    ...
    job_master_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("job_masters.job_master_id"), primary_key=True)
    skill_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("skills.skill_id"), primary_key=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(6), nullable=False)
    deleted_at: Mapped[Optional[datetime]] = mapped_column(DateTime(6), nullable=True)
...
class JobMaster(Base):
    ...
    # Primary Key
    job_master_id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
...
class JobPost(Base):
    ...
    # Primary Key
    job_post_id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)

    # Foreign Keys
    job_master_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("job_masters.job_master_id"), nullable=False, index=True)
    company_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("companies.company_id"), nullable=False, index=True)
    created_by: Mapped[Optional[int]] = mapped_column(BigInteger, ForeignKey("users.user_id"), nullable=True, index=True)
