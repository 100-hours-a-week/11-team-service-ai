...
class JobVectorRepository(BaseVectorRepository):
                ...
                self.client.collections.create(
                    ...
                    properties=[
                        Property(
                            ...
                            description="Job Master ID",
                            skip_vectorization=True,
                        ),
                        Property(
                            name="job_post_id",
                            data_type=DataType.INT,
                            description="Job Post ID (Specific Posting)",
                            skip_vectorization=True,
                        ),
                        Property(
                            name="company_id",
                        ...
                        Property(
                            ...
                            description="Full Job Data (Serialized JSON) - Embedding Target",
                        ),
                    ],
                )
    ...
    async def search_similar_jobs(
        self, company_id: int, query_text: str, limit: int = 3
    ) -> List[Dict[str, Any]]:
    ...
    async def add_job(
        self, job_master_id: int, job_post_id: int, company_id: int, content: str
    ):
    ...
    async def delete_jobs_by_master_id(self, job_master_id: int) -> int:
        ...
        try:
            ...
            logger = logging.getLogger(__name__)
            logger.error(
                f"❌ Failed to delete vectors for master id {job_master_id}: {e}"
            )
            return 0
