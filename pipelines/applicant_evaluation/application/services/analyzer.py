...
class ApplicationAnalyzer:
    ...
    async def run(self, user_id: int, job_id: int) -> PipelineEvaluateResponse:
        ...
        competency_results: List[CompetencyResult] = await asyncio.gather(*evaluation_tasks)

        # 5. 종합 평가 및 리포트 생성 (AI Synthesis -> Domain Factory)
        overall_feedback = await self.agent.synthesize_report(job_info, competency_results)
